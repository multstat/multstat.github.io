...
install.packages("DAMisc") ### Для загрузки могут понадобиться еще пакеты car, effects, rstan, QRM, StanHeaders
...
ggplot(pred.data, aes(x=respondentage, y=pred.logit)) +  
  geom_line(data=pred.data, aes(y=pred.logit)) +
...
# Wald test. Single coefficient 
wald.test(b = m1_logit$coefficients, Sigma = vcov(m1_logit), Terms = 2) #Gender
wald.test(b = m1_logit$coefficients, Sigma = vcov(m1_logit), Terms = 4) # Rural settlement
wald.test(b = m1_logit$coefficients, Sigma = vcov(m1_logit), Terms = 6) #Children

# Wald test. Multiple coefficients
wald.test(b = m1_logit$coefficients, Sigma = vcov(m1_logit), Terms = 4:5)
...
dc(m1_logit_, values1 = c(1, 1, mean(lits$respondentage), 0, 0, 0, mean(lits$ladder)),
   values2 = c(1, 1, mean(lits$respondentage), 0, 0, 1, mean(lits$ladder)))

dc(m1_logit_, values1 = c(1, 1, mean(lits$respondentage), 0, 0, 0, min(lits$ladder)),
   values2 = c(1, 1, mean(lits$respondentage), 0, 0, 0, max(lits$ladder)))

### Discrete change for many variables
glmChange(m1_logit_, data = lits, typical.dat=NULL, diffchange=c("range"), sim=T, R=1000) # min->max
glmChange(m1_logit_, data = lits, typical.dat=NULL, diffchange=c("sd"), sim=T, R=1000)    # -+std.deviation/2
...
# Odds ratio
summary(m1_logit_)
exp(cbind(m1_logit_$coefficients, confint(m1_logit_)))
