...
ME$money <- replace(ME$money, which(ME$money < 0), NA)
ME$money <- replace(ME$money, which(ME$money == 0), 0.1)
...
model1 <- lmer(votepct ~ log(money) + (1|state_id), REML = FALSE, data = ME)
summary(model1)
...
model2 <- lmer(votepct ~ log(money) + party + (1|state_id), REML = FALSE, data = ME)
summary(model2)
anova(model1, model2)

# add fixed effects for state-level predictors (acres)
model3 <- lmer(votepct ~ log(money) + party + acres + (1|state_id), REML = FALSE, data = ME)
summary(model3)
anova(model2, model3)

# add random effects
model4.1 <- lmer(votepct ~ log(money) + party + (1 + party|state_id), REML = FALSE, data = ME)
summary(model4.1) 
ranef(model4.1) 
...
model4.2 <- lmer(votepct ~ log(money) + party + (1 + money|state_id), REML = FALSE, data = ME)
...
model5 <- lmer(votepct ~ log(money) + party + acres + party*acres + (1 + party|state_id), REML = FALSE, data = ME)
summary(model5)

### diagnostics for variance components
model <- lmer(votepct ~  log(money) + party  + (1+party|state_id), REML = FALSE, data = ME)
...
model_nlme <- lme(votepct ~  log(money) + party, random = ~1+party|state_id, method = "ML", data = ME)
summary(model_nlme)
...
model_nlme_1 <- lme(votepct ~  log(money) + party, random = list(state_id = pdDiag(~1 + ~party)), method = "ML", data = ME)
